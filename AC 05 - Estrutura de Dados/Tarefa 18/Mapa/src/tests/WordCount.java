...
public class WordCount {
    public static void main(String[] args) throws IOException {
        ...
        while (doc.hasNext()) {
            ...
            if (word.length()<=3) continue; // ignora letras com 3 ou menos caracteres
            word = word.toLowerCase(); // ignora maiúscula e minúscula
            count = h.get(word); // pega o contador anterior e conta com esta palavra
            if (count == null) h.put(word, 1); // autoboxing permite isso
            else h.put(word, ++count); // autoboxing/unboxing permite isso
        }
