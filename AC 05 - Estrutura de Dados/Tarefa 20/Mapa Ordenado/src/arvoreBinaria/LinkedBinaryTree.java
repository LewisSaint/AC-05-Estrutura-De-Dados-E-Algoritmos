...
//-----------------------------------------Implementação dos Slides-----------------------------------------//
    ...
    public LinkedBinaryTree buildExpression(String e) {
        ...
        for (int i = 0; i < e.length(); i++) {
            if ((e.charAt(i) == '+') || (e.charAt(i) == '-') || (e.charAt(i) == '/') || (e.charAt(i) == '*') || (Character.isDigit(e.charAt(i)))) {
                LinkedBinaryTree tree = new LinkedBinaryTree();
                tree.addRoot(e.charAt(i));
                s.push(tree);
            } else if (e.charAt(i) == '(') {
                continue;
            } else {
                LinkedBinaryTree t2 = (LinkedBinaryTree) s.pop();
                LinkedBinaryTree t = (LinkedBinaryTree) s.pop();
                LinkedBinaryTree t1 = (LinkedBinaryTree) s.pop();
        ...
        return (LinkedBinaryTree) s.pop();

    }


    public int evaluateExpression(LinkedBinaryTree T, BTNode v) {

        if (v.getParent() == T.root()) {
            char[] operadores = {'+', '-', '/', '*'};
            char o;

            int x = evaluateExpression(T, (BTNode) T.left((Position) v));
            int y = evaluateExpression (T, (BTNode) T.right((Position) v));

            if (v.element() == "+") {return x + y;}
            else if (v.element() == "-") {return x - y;}
            else if (v.element() == "/") {return x / y;}
            else {return x * y;}
